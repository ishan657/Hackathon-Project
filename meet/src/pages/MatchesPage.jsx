...
const MatchesPage = ({ aiMatches = [], setPage, user }) => {
  ...
  const handleConnect = async (targetId) => {
    // Sanitize ID: Remove any non-hex characters Gemini might have added
...
/api/users/request/${cleanId}`, {}, {
  headers: { 'Authorization': `Bearer ${token}` }
      ...
      const msg = error.response?.data?.msg || error.response?.data?.error || "Server Error";
      alert(msg);
    ...
    <div className="max-w-5xl mx-auto pt-24 pb-12 px-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
        {aiMatches.map((match, index) => {
          const isSent = sentRequests.includes(match.id);
          ...
          return (
            ...
            >
              <div className="relative mb-4">
                <div className="w-16 h-16 rounded-full overflow-hidden border border-zinc-100 dark:border-zinc-800">
                  <img 
              ...
              <h3 className="text-md font-bold text-zinc-900 dark:text-zinc-100 mb-1 uppercase tracking-tight">{match.name}</h3>

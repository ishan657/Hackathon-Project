...
import { 
  Heart, Trophy, Tent, BookOpen, Clock, Code, ArrowRight, 
  Sparkles, Zap, Target, Search, Edit3, Navigation, User, Users,
  Gamepad2, Music, Camera, Coffee, Dumbbell, Palette, Mic2
} from 'lucide-react';
...
const Dashboard = ({ user, setPage, onStartExploring, setGlobalUser }) => {
  ...
  const [intent, setIntent] = useState(user?.lookingFor || "Exploring campus vibes");

  const categories = [
    { id: 'dating', title: 'Campus Dating', icon: <Heart size={18}/>, color: 'text-red-500' },
    { id: 'hackathon', title: 'Hackathon Partner', icon: <Trophy size={18}/>, color: 'text-amber-500' },
    { id: 'trip', title: 'Camp/Trip', icon: <Tent size={18}/>, color: 'text-emerald-500' },
    { id: 'study', title: 'Study Buddy', icon: <BookOpen size={18}/>, color: 'text-blue-500' },
    { id: 'night', title: 'Night Canteen', icon: <Clock size={18}/>, color: 'text-orange-500' },
    { id: 'coding', title: 'Coding Match', icon: <Code size={18}/>, color: 'text-zinc-600' },
    { id: 'gaming', title: 'Gaming Squad', icon: <Gamepad2 size={18}/>, color: 'text-purple-500' },
    { id: 'gym', title: 'Gym Partner', icon: <Dumbbell size={18}/>, color: 'text-slate-700' },
    { id: 'music', title: 'Jam Session', icon: <Music size={18}/>, color: 'text-pink-500' },
    { id: 'photo', title: 'Photography', icon: <Camera size={18}/>, color: 'text-cyan-500' },
    { id: 'coffee', title: 'Coffee Break', icon: <Coffee size={18}/>, color: 'text-yellow-700' },
    { id: 'art', title: 'Creative Arts', icon: <Palette size={18}/>, color: 'text-indigo-500' },
    { id: 'talk', title: 'Deep Talks', icon: <Mic2 size={18}/>, color: 'text-rose-500' },
  ];
    ...
    <div className="max-w-6xl mx-auto pt-32 pb-20 px-6 space-y-24 text-zinc-900 dark:text-zinc-100 font-sans">
      ...
      <div className="text-center space-y-10">
        <div className="space-y-4">
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-zinc-100 dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 animate-pulse">
            ...
            <span className="text-[10px] font-black uppercase tracking-[0.2em] text-zinc-500">Enhanced by Gemini AI</span>
          </div>
          
          <h1 className="text-6xl md:text-7xl font-light tracking-tight leading-tight">
            Find your <span className="font-black italic">tribe</span>, {user?.name?.split(' ')[0]}.<br/>
        ...
        <div className="flex flex-wrap justify-center gap-3 max-w-5xl mx-auto">
          {categories.map((cat) => (
            <div key={cat.id} className="relative group">
              ...
              >
                <span className={`${intent.startsWith(cat.title) ? 'text-white' : cat.color} transition-colors`}>
                  {cat.icon}
                </span>
                <span className="font-black text-[10px] uppercase tracking-[0.2em]">{cat.title}</span>
              </button>
      ...
      <div className="pt-10 border-t border-zinc-100 dark:border-zinc-800 flex flex-col md:flex-row justify-center items-center gap-12 opacity-50 grayscale hover:grayscale-0 transition-all duration-700">
         <div className="flex items-center gap-3">
            ...
            <span className="text-[10px] font-black uppercase tracking-widest">500+ NITians Online</span>
         </div>
         <div className="flex items-center gap-3">
            <Sparkles className="w-5 h-5 text-purple-500" />
            <span className="text-[10px] font-black uppercase tracking-widest">Powered by Gemini 1.5 Flash</span>
         </div>
      </div>
    </div>
  );

...
const ChatInterface = ({
  ...
  useEffect(() => {
    const fetchFriends = async () => {
      try {
        ...
        if (activeChatFriend) {
          const friendInList = res.data.find(
            (f) => f._id === activeChatFriend._id
          );
  ...
  useEffect(() => {
    ...
    const fetchMessages = async () => {
      try {
        ...
        const res = await axios.get(
          `${API_URL}/api/messages/${activeFriend.conversationId}`,
          {
            headers: { Authorization: `Bearer ${token}` },
          }
        );
    ...
    socket.on("receive_message", (data) => {
      ...
      if (
        data.conversationId === activeFriend.conversationId &&
          ...
          <>
            <div className="p-4 bg-white dark:bg-zinc-950 border-b border-zinc-200 dark:border-zinc-800 flex justify-between items-center">
              <div className="flex items-center gap-3">
                <img
                  src={`https://api.dicebear.com/7.x/avataaars/svg?seed=${activeFriend.name}`}
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-4">
              {chatHistory.map((msg, idx) => (
                ...
                >
                  <div
                    className={`max-w-[70%] px-4 py-2 rounded-2xl text-sm shadow-sm ${msg.sender === user._id ? "bg-zinc-900 text-white rounded-tr-none" : "bg-white dark:bg-zinc-800 text-zinc-900 dark:text-zinc-100 rounded-tl-none"}`}
                  >
                    {msg.text}
            ...
            <form
              ...
              className="p-4 bg-white dark:bg-zinc-950 border-t border-zinc-200 dark:border-zinc-800"
            >
              <div className="flex items-center gap-2 bg-zinc-100 dark:bg-zinc-900 p-2 rounded-2xl border border-zinc-200 dark:border-zinc-800">
                ...
                <button
                  type="submit"
                  className="bg-zinc-900 dark:bg-zinc-100 text-white dark:text-zinc-900 p-2.5 rounded-xl hover:scale-105 transition-all"
                >
                  <Send size={18} />
                </button>
              </div>
